...
spec = importlib.util.spec_from_file_location("automation", ROOT / "preprocessing/automate_DaudHidayatRamadhan.py")
auto = importlib.util.module_from_spec(spec)
spec.loader.exec_module(auto)

class CleaningTests(unittest.TestCase):
    def test_conflicts_remove_all_members_and_keep_literal_text(self):
        rows = [["same",1,0,0,0],["same",0,1,0,0],["duplicate",0,0,0,1],
                ["duplicate",0,0,0,1],["  Mixed CASE ; punctuation  ",1,0,0,0],
                [None,1,0,0,0],["   ",0,1,0,0],["multi",1,1,0,0],["zero",0,0,0,0]]
        clean, report = auto.clean_frame(pd.DataFrame(rows,columns=["Sentence"]+auto.LABELS))
        self.assertEqual(set(clean.Sentence), {"duplicate", "  Mixed CASE ; punctuation  "})
    ...
    def test_executed_notebook_parity_and_repeatability(self):
        expected = json.loads((ROOT / "tests/fixtures/manual-manifest.json").read_text())
        with tempfile.TemporaryDirectory() as temp:
            ...
            for name in ["train.csv.gz","test.csv.gz","manifest.json"]:
                self.assertEqual((Path(temp)/"first"/name).read_bytes(), (ROOT/"preprocessing/sqli_xss_preprocessing"/name).read_bytes())
