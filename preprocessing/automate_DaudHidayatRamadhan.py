...
def clean_frame(df):
    ...
    work["sample_id"] = work.Sentence.map(
...
def preprocess(
    raw_path=ROOT / "sqli_xss_raw/payloads-v2.csv.gz",
    output=ROOT / "preprocessing/sqli_xss_preprocessing",
    sample_size=24000,
    seed=42,
    expected_hash=RAW_SHA256,
):
    """Validate the raw snapshot, create deterministic splits, and write data plus hashes."""
    ...
    if raw_hash != expected_hash:
        raise ValueError(
            "Raw checksum changed; review the new source and update notebook and automation together"
        )
    ...
    selected, excluded = train_test_split(
        clean, train_size=sample_size, stratify=clean.target, random_state=seed
    )
    train, test = train_test_split(
        selected, test_size=0.2, stratify=selected.target, random_state=seed
    )
    train, test = [
        frame.sort_values("sample_id").reset_index(drop=True) for frame in (train, test)
    ]
    ...
    for name, frame in [("train", train), ("test", test)]:
        ...
        with destination.open("wb") as handle:
            with gzip.GzipFile(
                fileobj=handle, filename="", mode="wb", mtime=0
            ) as stream:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output", type=Path, default=ROOT / "preprocessing/sqli_xss_preprocessing"
    )
